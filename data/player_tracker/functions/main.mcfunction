...
execute if entity @e[type=item,nbt={Item:{id:"minecraft:compass", tag:{tracker:1b}}}] run function player_tracker:find_target

#check if there is no compass in "tracker"/"FFA" team's inventory and give them compass
execute unless entity @e[type=item,nbt={Item:{id:"minecraft:compass", tag:{tracker:1b}}}] run function player_tracker:give_compass